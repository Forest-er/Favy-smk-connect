...
<section class="py-16 px-6 md:px-12 lg:px-24 relative overflow-visible">
    ...
    <svg class="absolute top-16 left-1/3 w-12 h-12 opacity-45" viewBox="0 0 100 100">
        <polygon points="50,5 90,30 90,70 50,95 10,70 10,30" fill="none" stroke="#10b981" stroke-width="3" />
    </svg>

    <svg class="absolute top-1/4 right-1/4 w-32 h-32 opacity-40" viewBox="0 0 100 100">
        <path d="M10,50 Q30,20 50,50 T90,50" fill="none" stroke="#14b8a6" stroke-width="2.5" />
        <path d="M10,60 Q30,30 50,60 T90,60" fill="none" stroke="#14b8a6" stroke-width="2.5" />
    </svg>

    <div class="absolute top-1/2 right-1/3 w-3 h-3 bg-green-600/50 rounded-full"></div>
    <div class="absolute top-1/2 right-1/3 w-2 h-2 bg-teal-600/50 rounded-full" style="margin-top: 8px; margin-right: 24px;"></div>
    ...
    <div class="hidden md:block relative z-10" id="desktopCarousel">
        ...
        @foreach ($slides as $index => $slide)
            <div class="carousel-slide {{ $index === 0 ? 'active' : '' }} w-full relative z-10 mb-6">
                @if ($index > 0)
                ...
                <div class="flex gap-6 min-w-full px-4 md:px-6 overflow-visible">
                    @foreach ($slide as $service)
                        <div class="w-60 shrink-0 bg-white rounded-xl shadow-md hover:shadow-2xl hover:-translate-y-2 transition-transform duration-300 overflow-visible">
