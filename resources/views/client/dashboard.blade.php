...
<div class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    <div class="max-w-7xl mx-auto px-6 py-8">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div class="flex flex-col gap-3">
                        <div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 text-white">
                            ...
                            <p class="text-sm mt-1"><span class="text-green-300">↑ 12%</span> this month</p>
                        </div>
                    </div>
                </div>
        ...
        <section class="mb-12">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div class="stat-card bg-white rounded-2xl p-6 shadow-lg card-hover border border-gray-100">
                    ...
                    <p class="text-gray-500 text-sm mb-1 font-medium">Projects Posted</p>
                    <p class="text-4xl font-bold text-gray-900 mb-2">{{ $totalTasks }}</p>
                    <p class="text-xs text-gray-500">{{ $totalTasks - 2 }} in progress</p>
                </div>
                ...
                <div class="stat-card bg-white rounded-2xl p-6 shadow-lg card-hover border border-gray-100">
                    ...
                    <p class="text-gray-500 text-sm mb-1 font-medium">Total Spent</p>
                    <p class="text-4xl font-bold text-gray-900 mb-2">Rp5M</p>
                    <p class="text-xs text-gray-500">Rp1.2M this month</p>
                </div>
                ...
                <div class="stat-card bg-white rounded-2xl p-6 shadow-lg card-hover border border-gray-100">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-1 rounded-full">Verified</span>
                    </div>
                    <p class="text-gray-500 text-sm mb-1 font-medium">Active Freelancers</p>
                    <p class="text-4xl font-bold text-gray-900 mb-2">7</p>
        ...
        <section class="mb-12">
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-1">Browse freelancers by expertise</p>
                </div>
        ...
        <section class="mb-12">
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-1">Your active projects</p>
                </div>
            </div>

            <div class="flex gap-6 overflow-x-auto scrollbar-hide pb-4">
                @foreach ($myTasks as $mt)
                    <div class="relative w-72 flex-shrink-0 rounded-2xl overflow-hidden shadow-lg group cursor-pointer border border-gray-100">
                        <img 
                            src="{{ asset('storage/' . $mt->foto) }}" 
                            alt="{{ $mt->judul }}" 
                        ...
                        <div class="absolute inset-0 flex flex-col justify-end p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            <h3 class="text-white text-lg font-bold mb-2">{{ $mt->judul }}</h3>
                            <p class="text-white/80 text-sm">Click to view details</p>
                        </div>
                        
                        <div class="absolute top-4 right-4">
                            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-white/90 text-gray-800">
                                In Progress
                            </span>
                        </div>
                    </div>
        ...
        <section class="mb-12">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-900 mb-2">Get Inspired by Amazing Work</h2>
                <p class="text-gray-500 text-sm">Discover portfolios from talented freelancers</p>
            </div>
            ...
            <div class="flex gap-3 overflow-x-auto scrollbar-hide mb-8">
                @foreach ($jurusans as $index => $jurusan)
                    @php $gradient = $gradients[$index % count($gradients)]; @endphp
                    <button 
                        class="px-5 py-2.5 rounded-full text-gray-700 text-sm font-medium border border-gray-300 bg-white transition-all duration-300 hover:text-white hover:border-transparent hover:bg-gradient-to-r hover:{{ $gradient['from'] }} hover:{{ $gradient['via'] }} hover:{{ $gradient['to'] }} hover:shadow-lg whitespace-nowrap">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px]">
                @foreach ($tasks as $task)
                    <div class="relative group cursor-pointer rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow border border-gray-100
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
                                <p class="font-semibold text-sm mb-1">{{ $task->judul }}</p>
                                <p class="text-xs text-white/80">by {{ $task->user->nama ?? 'Freelancer' }}</p>
                            </div>
                        </div>

                        <button class="absolute top-3 right-3 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:bg-pink-500 hover:text-white">
